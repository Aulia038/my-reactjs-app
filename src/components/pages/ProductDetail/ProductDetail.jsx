    ...
    <>
      ...
      <div className="flex gap-[30px] px-24">
        <div className="w-500px">
          ...
          <img
            src={product.detailImages ? product.detailImages[activeImage] : product.imageUrl}
            alt={product.name ?? "No Name"}
            className="block w-[800px] h-[700px] object-cover font-serif border-1 border-gray-300 rounded-lg transition-transform duration-300 hover:scale-105"
          />
            ...
            {product.detailImages &&
              product.detailImages.map((image, index) => (
                <button
                  ...
                  className={`w-[130px] h-[130px] border-1 mb-5 rounded-md overflow-hidden transition-transform duration-300 hover:scale-105 ${index === activeImage ? "border-red-500" : "border-gray-300"}`}
                >
        ...
        <div className="flex flex-col gap-[20px]">
          ...
          {product.stock > 0 ? (
            <div>
              <Button type="button" className="inline-flex items-center justify-center gap-2 p-2 rounded-sm font-serif bg-[#DB4444] text-center hover:bg-[#5969cf] text-white active:bg-[#4956ab]">
